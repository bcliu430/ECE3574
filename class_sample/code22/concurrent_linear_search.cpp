...
const int searchfor[3] = {-500, 234234234, 567567565};

template<typename T>
bool linear_search(const std::vector<T> & list, const T & value)
{ 
...
{  
  std::future<bool> fut0 = std::async(&linear_search<int>, data, searchfor[0]);
  std::future<bool> fut1 = std::async(&linear_search<int>, data, searchfor[1]);
  std::future<bool> fut2 = std::async(&linear_search<int>, data, searchfor[2]);
...
void serial(const std::vector<int> & data)
{
  ...
  value0 = linear_search(data, searchfor[0]);
  value1 = linear_search(data, searchfor[1]);
  value2 = linear_search(data, searchfor[2]);
